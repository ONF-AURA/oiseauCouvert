...
cv_insolation <- function(dos_proj = oiseauData::data_conf("dos_projet"),
  ...
  mnt <- terra::rast(path_mnt) %>% terra::crop(mnh[[1]])
  ...
  ls <- purrr::map(names(mnh), function(n){
    ...
    insol <- cv.sun(oiseauUtil::util_spat2rast(mnt + mnhn))
